...
export default function Home() {
  ...
  const [selectedStateIndex, setSelectedStateIndex] = useState(25); // Default: Uttar Pradesh
  const [selectedCity, setSelectedCity] = useState("Ghaziabad");
  const [cityDirectorySearch, setCityDirectorySearch] = useState("");
  ...
  return (
    <>
      ...
      <main className={`${plusJakartaSans.variable} ${playfairDisplay.variable} min-h-screen overflow-x-hidden font-[var(--font-plus-jakarta)] text-[#101828] antialiased`}>
        ...
        {/* ==========================================================================
            9D. PAN-INDIA DIGITAL MARKETING & LOCAL SEO DIRECTORY (#coverage)
            ========================================================================== */}
        <section id="coverage" className="py-20 sm:py-24 bg-gradient-to-b from-[#f8faff] via-white to-slate-50/70 border-b border-slate-200">
          <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center max-w-[920px] mx-auto mb-14">
              <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-blue-50 border border-blue-200 text-[#1570ef] text-xs font-bold tracking-wider uppercase mb-3.5">
                <span className="w-2 h-2 rounded-full bg-[#1570ef] animate-pulse" />
                All 28 States &amp; 8 Union Territories • 300+ Cities Covered
              </div>
              <h2 className="text-[30px] sm:text-[42px] lg:text-[48px] font-extrabold text-[#080d24] tracking-tight leading-[1.14]">
                Pan-India Digital Marketing, SEO &amp;{" "}
                <span className="webfx-serif text-[#207de9] block sm:inline font-normal">
                  Local Search Domination
                </span>
              </h2>
              <p className="mt-4 text-sm sm:text-base text-slate-600 font-normal leading-relaxed max-w-[800px] mx-auto">
                No matter where your customers are located — in major metropolitan tech hubs or fast-growing commercial districts across India — Digital FX deploys hyper-local Google Maps 3-Pack optimization, high-converting Next.js web architectures, and performance paid ads that drive verified customer inquiries directly to your business.
              </p>
            </div>

            {/* Interactive City & State Growth Strategy Explorer */}
            <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-xl max-w-[1100px] mx-auto">
              <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 pb-8 border-b border-slate-100">
                <div>
                  <span className="text-[11px] font-bold uppercase tracking-wider text-[#207de9] bg-blue-50 px-3 py-1 rounded-full border border-blue-100">
                    City-Level Search Intent Engine
                  </span>
                  <h3 className="text-2xl sm:text-3xl font-black text-[#080d24] mt-2.5 tracking-tight">
                    Select Your State &amp; City in India
                  </h3>
                  <p className="text-xs sm:text-sm text-slate-500 font-normal mt-1">
                    See the exact high-converting keywords and ranking strategy deployed for your target market.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full lg:w-auto shrink-0">
                  ...
                  <div className="w-full sm:w-56">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const st = INDIA_STATES_AND_UTS[idx];
                        if (st && st.cities.length > 0) {
                          setSelectedCity(st.cities[0]);
                        }
                      }}
                      className="w-full h-11 px-3 rounded-xl border border-slate-200 bg-slate-50 hover:bg-white text-xs sm:text-sm font-bold text-[#080d24] focus:outline-none focus:border-[#207de9] transition cursor-pointer shadow-xs"
                    >
                      {INDIA_STATES_AND_UTS.map((item, idx) => (
                        <option key={idx} value={idx}>
                          {item.name} ({item.cities.length} {item.cities.length === 1 ? "city" : "cities"})
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="w-full sm:w-56">
                    ...
                    <select
                      ...
                      className="w-full h-11 px-3 rounded-xl border border-slate-200 bg-slate-50 hover:bg-white text-xs sm:text-sm font-bold text-[#080d24] focus:outline-none focus:border-[#207de9] transition cursor-pointer shadow-xs"
                    >
                      {(INDIA_STATES_AND_UTS[selectedStateIndex]?.cities || []).map((city, idx) => (
              ...
              {/* Strategy Results & Keywords Card for Selected City */}
              <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                <div className="lg:col-span-7 space-y-4">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse" />
                    <h4 className="text-lg sm:text-xl font-extrabold text-[#080d24] tracking-tight">
                      Active Campaign Coverage for <span className="text-[#207de9]">{selectedCity}</span>, {INDIA_STATES_AND_UTS[selectedStateIndex]?.name}
                    </h4>
                  </div>
                  <p className="text-xs sm:text-[13px] text-slate-600 font-normal leading-relaxed">
                    Digital FX optimizes local business listings in <strong className="text-[#080d24]">{selectedCity}</strong> using Google Business Profile verification, high-DA citation syndication across 50+ business portals, and hyper-targeted Google Ads to capture buyers searching for services in your area.
                  </p>

                  {/* High Intent Customer Keywords Chips */}
                  <div className="pt-2">
                    <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2.5">
                      Target High-Converting Customer Search Queries in {selectedCity}:
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {[
                        `best digital marketing agency in ${selectedCity}`,
                        `digital marketing agency near me in ${selectedCity}`,
                        `top SEO company in ${selectedCity}`,
                        `Google Maps ranking agency ${selectedCity}`,
                        `Google Ads PPC management ${selectedCity}`,
                        `website development company in ${selectedCity}`,
                        `lead generation agency ${selectedCity}`,
                        `performance marketing in ${selectedCity}`,
                      ].map((kw, idx) => (
                        <span
                          key={idx}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200/80 text-[11px] font-semibold text-slate-700 hover:border-[#207de9] hover:text-[#207de9] transition-colors"
                        >
                ...
                <div className="lg:col-span-5 bg-gradient-to-br from-[#080d24] to-[#12204a] text-white p-6 sm:p-7 rounded-2xl shadow-xl flex flex-col justify-between space-y-5">
                  <div>
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-white/10 text-emerald-400 text-[10px] font-bold uppercase tracking-wider mb-2.5 border border-white/10">
                      <span>⚡ Instant Action</span>
                    </div>
                    <h5 className="text-lg font-black text-white leading-tight">
                      Rank #1 on Google in {selectedCity}
                    </h5>
                    <p className="text-xs text-slate-300 font-normal mt-2 leading-relaxed">
                      Run our live AI-powered SEO Audit for {selectedCity} or request a tailored local revenue proposal.
                    </p>
                  </div>
                  ...
                  <div className="space-y-2.5 pt-2">
                    <button
                      ...
                      className="w-full py-3 px-4 rounded-xl bg-[#207de9] hover:bg-[#1a6bc7] text-white font-bold text-xs sm:text-sm text-center transition-all shadow-md shadow-blue-500/25 cursor-pointer flex items-center justify-center gap-2"
                    >
                      <span>Run Free AI Audit for {selectedCity}</span>
                      <span>→</span>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={scrollToContact}
                      className="w-full py-2.5 px-4 rounded-xl bg-white/10 hover:bg-white/15 text-white font-semibold text-xs text-center border border-white/15 transition-all cursor-pointer"
                    >
              ...
              {/* Expandable All-India State & City Directory Grid */}
              <div className="mt-10 pt-8 border-t border-slate-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h4 className="text-base font-extrabold text-[#080d24]">
                      Explore All 28 States &amp; 8 Union Territories
                    </h4>
                    <p className="text-xs text-slate-500 font-normal mt-0.5">
                      Click any city pill below to preview tailored marketing and local SEO strategy.
                    </p>
                  </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-100 space-y-6 animate-fadeIn">
                    ...
                    <div className="max-w-md">
                      <div className="relative">
                        <input
                          type="text"
                          value={cityDirectorySearch}
                          onChange={(e) => setCityDirectorySearch(e.target.value)}
                          placeholder="Search any city in India (e.g., Surat, Indore, Patna, Kochi)..."
                          className="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-200 bg-slate-50/60 focus:bg-white text-xs sm:text-sm font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#207de9] transition"
                        />
                    ...
                    {/* Grid of all states and their cities */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 max-h-[560px] overflow-y-auto pr-2 custom-scrollbar">
                      {INDIA_STATES_AND_UTS
                        .filter((st) => {
                          if (!cityDirectorySearch) return true;
                          const q = cityDirectorySearch.toLowerCase();
                          return (
                            st.name.toLowerCase().includes(q) ||
                            st.cities.some((c) => c.toLowerCase().includes(q))
                          );
                        })
                        .map((st, idx) => {
                          const stateOriginalIndex = INDIA_STATES_AND_UTS.findIndex((item) => item.name === st.name);
                          const matchingCities = cityDirectorySearch
                            ? st.cities.filter((c) => c.toLowerCase().includes(cityDirectorySearch.toLowerCase()))
                            : st.cities;

                          return (
                            <div
                              key={idx}
                              className="p-4 rounded-2xl bg-slate-50/80 border border-slate-200/90 hover:border-blue-200 transition flex flex-col justify-between"
                            >
                              <div>
                                <div className="flex items-center justify-between gap-2 mb-2.5">
                                  <span className="text-[13.5px] font-extrabold text-[#080d24]">
                                    {st.name}
                                  </span>
                                  <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-white border border-slate-200 text-slate-500">
                                    {st.type === "state" ? "State" : "UT"}
                                  </span>
                                </div>
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                  {matchingCities.map((city, cIdx) => (
                                    <button
                                      key={cIdx}
                                      type="button"
                                      onClick={() => {
                                        setSelectedStateIndex(stateOriginalIndex);
                                        setSelectedCity(city);
                                        const el = document.getElementById("coverage");
                                        if (el) el.scrollIntoView({ behavior: "smooth" });
                                      }}
                                      className={`text-[11px] font-semibold px-2 py-1 rounded-md transition cursor-pointer ${
                                        selectedCity === city
                                          ? "bg-[#207de9] text-white shadow-xs"
                                          : "bg-white hover:bg-blue-50 text-slate-700 border border-slate-200/80 hover:border-blue-200 hover:text-[#207de9]"
                                      }`}
                                    >
                                      {city}
                                    </button>
